...
class FractureBis(object):
    def __init__(self, specimen : Specimen, elastic : ElasticRegion, ld : LoadDisplacement, id_computation : int):
        ...
        self.P = self.ld.load[self.id_computation]
        load, disp = self.ld.get_LD_sorted()
        self.intercept_2 = -self.elastic.stiffness*disp[self.id_computation] + load[self.id_computation]
        self.A_pl = np.trapz(load[:self.id_computation], disp[:self.id_computation])
        self.min_disp = np.min(disp)
    ...
    def stress_intensity_factor(self) -> float:
        ...
        self.K = self.P*self.specimen.S/(np.sqrt(self.specimen.B*self.specimen.B_N) * self.specimen.W**1.5) * f_geom
    ...
    def J_integral_el(self) -> float:
        self.J_el = self.K**2 * (1.0 - self.specimen.nu**2) / self.specimen.E
    ...
    def J_integral_pl(self) -> float:
        ...
        self.J_pl = self.specimen.eta_pl*A_pl/(self.specimen.B_N*self.specimen.b0)
